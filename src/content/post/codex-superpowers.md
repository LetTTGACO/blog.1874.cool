...
updated: '2026-07-14 18:48:00'
draft: false
---
> 目前 GPT 5.6 的子代理调用存在 BUG，需等待官方解决后再使用，具体 Issue：[https://github.com/openai/codex/issues/32291](https://github.com/openai/codex/issues/32291)
...
再加上刚更新的 5.6 的 Sol、Terra、Luna 模型让我有了更多选择，而不是一味的开最高档进行日常和开发。
...
虽然 Superpowers 内置的 `subagent-driven-development`有一套完整的模型选择方案，用来告诉 AI Agent 在不同情况下选择什么样的模型来执行任务，但由于 Superpowers 并不是 Codex 专属 Skill，它需要同时服务多个 AI Agent， 所以在指定模型时，它的用词都比较通用：`cheapest models`、`standard model`、`most capable model`。而实际 AI Agent 在分配子代理模型时，大多时候都比较模糊，并不一定控制的非常精确。而这也是我要优化用来省 Token 的关键点。


我让 Codex 分析我过往一些 Superpowers 生成的实施计划，给出的答案很惊人：几乎 60-70% 的 Task 都可以用`cheapest models`来完成。这些 Task 都有一些共通点：
...
而这正是`cheapest models`的优势所在，这些代码实现并不需要多强的推理，而且 cheap 与文件数量无关，边界清楚时跨多个文件也可以用。


再加上刚更新的 5.6 的 Sol、Terra、Luna 模型也天然适配这三种档位。
...
1. 重写/补强`subagent-driven-development`的 `Model Selection`模块，加入三档 tier、Codex 映射、plan 详细度判断、final reviewer 固定 `gpt-5.6-sol xhigh`。
2. 在子代理创建模板中指定 `reasoning_effort`推理程度。
3. 在`requesting-code-review`的 subagent 创建模板中指定使用`gpt-5.6-sol xhigh`来 review。
...
flowchart TD
  ...
  B -->|是| F["gpt-5.6-sol + xhigh"]
  B -->|否| C{是 task reviewer?}
  C -->|普通 review| R1["gpt-5.6-terra + high"]
  C -->|高风险 review| R2["gpt-5.6-sol + high"]
  C -->|否，implementation 或 fix| D{brief 是否闭合且机械?}
  D -->|是| I1["gpt-5.6-luna + medium"]
  D -->|否| E{是否需要架构/设计/推理型重试?}
  E -->|是| I3["gpt-5.6-sol + high"]
  E -->|否或不确定| I2["gpt-5.6-terra + medium"]
...
ChatGPT Pro 会送一个 [gpt-5.3-codex-spark](https://openai.com/zh-Hans-CN/index/introducing-gpt-5-3-codex-spark/) 的单独计费额度，也是同样有 5 小时额度和周额度，和正常的额度是分开计算的。其实可以将 `gpt-5.6-luna medium`替换为`gpt-5.3-codex-spark high`应该也问题不大，但毕竟 `gpt-5.6-luna` 足够便宜，先试用一段时间，`gpt-5.3-codex-spark`作为备选。


而且随着国产模型的崛起，我感觉这个流程中的很多模型其实用 GLM/DeepSeek 来实现，但奈何我目前没有找到一个相对比较简单稳定的跨模型/Agent方案来实现，所以只能先全程用 Codex 来完成。
